...
app.get('/team_stats', function(req, res) {
	var query1 = 'select visitor_name, game_date, home_score, visitor_score from football_games;';
...
app.get('/player_info/select_player', function(req, res) {
	var query1 = 'select * from football_players where id = ' + req.query.player_choice + ';'
	var query2 = 'SELECT id, name FROM football_players;';
	var query3 = 'select count(football_games.players) from football_games WHERE' + req.query.player_choice + ') = any(players);';
	db.task('get-everything', task => {
	    return task.batch([
		task.any(query1),
		task.any(query2),
